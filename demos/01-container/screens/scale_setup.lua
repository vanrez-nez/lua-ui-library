...
local function find_required(root, id)
    ...
    if node == nil then
        error('scale_setup: missing node "' .. id .. '"', 2)
    end
...
function Setup.install(args)
    ...
    local single_parent = find_required(root, 'scale-single-parent')
    local single_child = find_required(root, 'scale-single-child')
    local nested_parent = find_required(root, 'scale-nested-parent')
    local nested_child = find_required(root, 'scale-nested-child')
    local nested_grandchild = find_required(root, 'scale-nested-grandchild')
    ...
    rawset(stage, '_demo_screen_hooks', {
        update = function(dt)
            ...
            single_parent.y = center_y + helpers.round(math.sin(elapsed * 0.9) * 20)

            nested_parent.x = base_x + single_parent.width + gap
            nested_parent.y = center_y + helpers.round(math.cos(elapsed * 1.05) * 20)
