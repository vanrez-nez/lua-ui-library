...
local memoize_tick = Memoize.memoize_tick or function(fn)
    return fn
end
local memoize_global_tick = Memoize.tick or function()
end
...
function Stage:__newindex(key, value)
    ...
    if declared_props ~= nil and declared_props[key] ~= nil then
        rawset(self, key, value)
        return
    end
...
local synchronize_for_read = memoize_tick(function(self)
