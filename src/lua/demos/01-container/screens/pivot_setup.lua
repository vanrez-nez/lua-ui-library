...
local function find_required(root, id)
    ...
    if node == nil then
        error('pivot_setup: missing node "' .. id .. '"', 2)
    end
...
function Setup.install(args)
    ...
    local single_parent = find_required(root, 'pivot-single-parent')
    local single_child = find_required(root, 'pivot-single-child')
    local nested_parent = find_required(root, 'pivot-nested-parent')
    local nested_child = find_required(root, 'pivot-nested-child')
    local nested_grandchild = find_required(root, 'pivot-nested-grandchild')
