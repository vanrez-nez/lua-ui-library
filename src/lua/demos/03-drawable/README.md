...
Build the focused `Drawable` demo without repeating behavior already covered by `src/lua/demos/01-container` or the retained graphics surfaces now isolated in `src/lua/demos/04-graphics`.
...
Those are already owned by `src/lua/demos/01-container`.
...
- Graphics-specific retained compositing behavior now lives in `src/lua/demos/04-graphics`.
