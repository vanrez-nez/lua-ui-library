...
CircleShape._allow_rect_draw_fallback = false
...
local function resolve_stroke_draw_options(shape, bounds, world_points)
    ...
    if stroke_pattern == 'dashed' and gap_length > 0 then
        ...
        local cycle = dash_length + gap_length
        local normalized_offset = MathUtils.positive_mod(dash_offset, cycle)
        local shifted_seam = MathUtils.positive_mod(
...
function CircleShape:_get_local_points(out_table)
...
function CircleShape:_draw_fill(graphics, active_fill)
    ...
    if active_fill.kind == 'color' and axis_aligned_world_ellipse ~= nil then
        return draw_axis_aligned_ellipse_fill(
            graphics,
    ...
    return Shape._draw_fill(self, graphics, active_fill)
end

function CircleShape:_draw_stroke(graphics)
