...
local function effective_duration(self)
    local duration = rawget(self, 'duration')
    if rawget(self, 'closeMethod') == 'auto-dismiss' and duration == nil then
        return 5000
    end
    return duration
end
...
local function axis_group_key(self)
    return table.concat({
        rawget(self, 'edge'),
        rawget(self, 'align'),
        tostring(rawget(self, 'safeAreaAware')),
    }, ':')
end
...
local function collect_notification_surfaces(stage, group_key, out)
    ...
    for index = 1, #children do
        ...
        if owner ~= nil and effective_open(owner) and axis_group_key(owner) == group_key and rawget(owner, 'stackable') == true then
            out[#out + 1] = owner
        end
    end
    return out
...
local function position_surface(self, stage)
    ...
    if rawget(self, 'edge') == 'bottom' then
        y = viewport.y + viewport.height - surface_height - margin
    elseif rawget(self, 'edge') == 'right' then
        x = viewport.x + viewport.width - surface_width - margin
    end

    if rawget(self, 'edge') == 'top' or rawget(self, 'edge') == 'bottom' then
        if rawget(self, 'align') == 'center' then
            x = viewport.x + (viewport.width - surface_width) * 0.5
        elseif rawget(self, 'align') == 'end' then
            x = viewport.x + viewport.width - surface_width - margin
        end
    else
        if rawget(self, 'align') == 'center' then
    ...
    if rawget(self, 'stackable') == true then
        ...
        for index = 1, #group do
            ...
            if rawget(self, 'edge') == 'top' then
                offset = offset + peer_bounds.height + 12
            elseif rawget(self, 'edge') == 'bottom' then
                offset = offset + peer_bounds.height + 12
            elseif rawget(self, 'edge') == 'left' then
...
function Notification:constructor(opts)
    ...
    rawset(self, 'closeMethod', opts.closeMethod or 'button')
    rawset(self, 'duration', opts.duration)
    rawset(self, 'stackable', opts.stackable ~= false)
    rawset(self, 'edge', opts.edge or 'top')
...
function Notification:update(dt)
    ...
    if wants_open and rawget(self, 'closeMethod') == 'auto-dismiss' then
